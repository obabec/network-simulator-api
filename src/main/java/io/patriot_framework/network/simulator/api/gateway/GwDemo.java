...
public class GwDemo {
        ...
        public void startGateway() throws InterruptedException {
            ...
            appManager.newApp(name)
                    ...
                    .add(Paths.get("start_daemons"),
                            new File("start_daemons").toPath())
                    .run("chmod +x start_daemons")
                    .cmd(Arrays.asList("java", "-jar", "virtual-smart-home/target/smart-home-virtual-1.0-SNAPSHOT.jar"))
                    .entrypoint(Arrays.asList("/start_daemons"));

            appManager.setTag("app_gateway");
            AppConfig appConfig = appManager.deploy(name, Arrays.asList("start_daemons"));
